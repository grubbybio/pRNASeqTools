...
if (-d $bowtie_index){
	system `mv $opt_n.* ./$bowtie_index/`;
}

my $cleaveland_out = "Cleaveland_output";
if (-d $cleaveland_out){
	
	system `mv $opt_e\_* ./$cleaveland_out/`;
	system `mv $opt_u\_G* ./$cleaveland_out/`;
}else{
	system `mkdir $cleaveland_out`;
...
sub check_GSTAr_version {
    ...
    (open(GS, "perl GSTAr.pl -v 2>&1 |")) || return 0;
    my $version = <GS>;
    close GS;
...
#    my $log = "$fasta" . "_bowtie_build_log.txt";
    my $bwt_ret = `bowtie-build $fasta $fasta`;
    unless($opt_q) {
	print STDERR " Done\n";
