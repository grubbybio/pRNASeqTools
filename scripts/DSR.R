...
for (n in 1:length(genotype)){
  for (k in 1:replicates[n]){
    ...
    genotype[n] -> b[p,]
    assign(rownames(b)[p], read.table(paste(genotype[n],"_",k,".count",sep = ""), as.is = T, row.names = 1)[,1:8])
  }
}
b -> ss
for(n in 1:p){
...
try({
    ...
    pdf(file=paste(method,"bin","all","pdf",sep="."),6,6)
    ...
    for(j in 2:length(genotype)){
        ...
        write.csv(out,paste(genotype[j],"vs",genotype[1],method,"bin","all","csv",sep="."),quote=F)
        write.csv(resup,paste(genotype[j],"vs",genotype[1],method,"bin","all","hyper","csv",sep="."),quote=F)
        write.csv(resdown,paste(genotype[j],"vs",genotype[1],method,"bin","all","hypo","csv",sep="."),quote=F)
    }
}, silent = T)
...
for(i in 2:8){
  ...
  try({
      ...
      rownames(sampleDistMatrix) <- rownames(b)
      colors <- colorRampPalette( rev(brewer.pal(9,"Blues")) )(255)
      pdf(file=paste(method,"bin",i+17,"pdf",sep="."),6,6)
      aheatmap(log2.norm.counts, Rowv=NA, Colv=NA, annCol=df)
      message("Top 1000 completed!")
      print(plotPCA(rld,ntop=1000,intgroup=c("condition")))
      message("PCA completed!")
      ...
      for(j in 2:length(genotype)){
          ...
          subset(out, pvalue < pvalueoo & log2FoldChange >= log2(foldchangeoo)) -> resup
          subset(out, pvalue < pvalueoo & log2FoldChange <= -log2(foldchangeoo)) -> resdown
          write.csv(out,paste(genotype[j],"vs",genotype[1],method,"bin",i+17,"csv",sep="."),quote=F)
          write.csv(resup,paste(genotype[j],"vs",genotype[1],method,"bin",i+17,"hyper","csv",sep="."),quote=F)
          write.csv(resdown,paste(genotype[j],"vs",genotype[1],method,"bin",i+17,"hypo","csv",sep="."),quote=F)
      }
  }, silent = T)
}
