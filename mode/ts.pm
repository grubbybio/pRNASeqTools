...
command_short_description q[Analysis for TS-CLIP-seq];
    ...
    for(my $i=0;$i<=$#tags;$i++){
      ...
      print $main::tee "\nMapping $tag...\n";
	      ...
	      if($#files == 0){
					...
					Function->rmvc($tag);
		  		system ("STAR --genomeDir Genome --alignIntronMax 5000 --outSAMtype BAM SortedByCoordinate --limitBAMsortRAM 10000000000 --outReadsUnmapped Fastx --outSAMmultNmax 1 --outFilterMismatchNoverLmax 0.1 --runThreadN ".$thread." --readFilesIn ".$tag.".fastq 2>&1");
          ...
          unlink ($tag.".fastq", $tag."_trimmed.fastq");
	        ...
	        system ("cutadapt -j ".$thread." -m 20 --trim-n -a ".$adaptor." -A AAAAAAGAAAAAA -o ".$tag."_R1_trimmed.fastq -p ".$tag."_R2_trimmed.fastq ".$tag."_R1.fastq ".$tag."_R2.fastq 2>&1");
					Function->rmvc($tag."_R1", $tag."_R2");
	        system ("STAR --genomeDir Genome --alignIntronMax 5000 --outSAMtype BAM SortedByCoordinate --limitBAMsortRAM 10000000000 --outReadsUnmapped Fastx --outSAMmultNmax 1 --outFilterMismatchNoverLmax 0.1 --runThreadN ".$thread." --readFilesIn ".$tag."_R1.fastq ".$tag."_R2.fastq 2>&1");
...
sub ana {
  ...
  system ("cat ".$command."\| sort -k1,1 -k2,2n \| bedtools merge -c 4 -o collapse -s -i - > tmp.bed");
  ...
  while(my $dd = <PEAK>){
    ...
    print FI "$row[0]\t$row[1]\t$row[2]\t$OutputGene\t$sum\t$row[3]\n" if $sum >= 10;
  }
  close PEAK;
  close FI;
