...
settings(mrms) <- list(minScoreMean = 0.93, minScoreA = 0.5, minCoverage = as.integer(args[2]))
modify(mrms,force = TRUE) -> mrms
sink(paste(args[1],"ScoreA","wig",sep="."))
for(i in 1:length(ranges(mrms))){
  cat(paste("variableStep chrom=",as.character(seqnames(ranges(mrms)))[i],"\n",sep=""))
  values(getDataTrack(mrms,i,type = c("scoreA", "scoreMean"))$scoreRMS) -> tmp
...
for(i in 1:length(ranges(mrms))){
  cat(paste("variableStep chrom=",as.character(seqnames(ranges(mrms)))[i],"\n",sep=""))
  values(getDataTrack(mrms,i,type = c("scoreA", "scoreMean"))$scoreMean) -> tmp
  for(j in 1:width(ranges(ranges(mrms))[[i]])){
    if(tmp[,j]>0){
      cat(paste(j,"\t",tmp[,j],"\n",sep=""))
    }
