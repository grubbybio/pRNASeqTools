...
for (n in 1:length(genotype)){
  for (k in 1:replicates[n]){
    ...
    genotype[n] -> b[p,]
    assign(rownames(b)[p], read.table(paste(genotype[n],"_",k,".gene.count",sep = ""), as.is = T, row.names = 1)[,1:8])
  }
}
b -> ss
for(n in 1:p){
...
try({
    ...
    for(j in 2:length(genotype)){
        ...
        write.csv(out,paste(genotype[j],"vs",genotype[1],method,"gene","all","csv",sep="."),quote=F)
        write.csv(resup,paste(genotype[j],"vs",genotype[1],method,"gene","all","hyper","csv",sep="."),quote=F)
        write.csv(resdown,paste(genotype[j],"vs",genotype[1],method,"gene","all","hypo","csv",sep="."),quote=F)
    }
}, silent = T)
...
for(i in 2:8){
  ...
  try({
      ...
      for(j in 2:length(genotype)){
          ...
          subset(out, pvalue < pvalueoo & log2FoldChange >= log2(foldchangeoo)) -> resup
          subset(out, pvalue < pvalueoo & log2FoldChange <= -log2(foldchangeoo)) -> resdown
          write.csv(out,paste(genotype[j],"vs",genotype[1],method,"gene",i+17,"csv",sep="."),quote=F)
          write.csv(resup,paste(genotype[j],"vs",genotype[1],method,"gene",i+17,"hyper","csv",sep="."),quote=F)
          write.csv(resdown,paste(genotype[j],"vs",genotype[1],method,"gene",i+17,"hypo","csv",sep="."),quote=F)
      }
  }, silent = T)
}
