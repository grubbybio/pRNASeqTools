...
for (n in 1:length(genotype1_tags)){
  for (k in 1:genotype1_reps[n]){
    ...
    assign(rownames(b)[q], read.table(paste(genotype1,"_",genotype1_tags[n],"_",k,".miRNA.annotated.count",sep = ""), header = T, as.is = T, row.names = 1))
  }
}
for (n in 1:length(genotype2_tags)){
  for (k in 1:genotype2_reps[n]){
    ...
    assign(rownames(b)[q], read.table(paste(genotype2,"_",genotype2_tags[n],"_",k,".miRNA.annotated.count",sep = ""), header = T, as.is = T, row.names = 1))
  }
}
for(n in 1:p){
...
message("Processing annotated length...")
apply(eval(parse(text=rownames(b)[1])),1,sum) -> a
for(q in 2:p){
  cbind(a,apply(eval(parse(text=rownames(b)[q])),1,sum)) -> a
}
...
try({
    ...
    pdf(file=paste(method,"miRNA","annotated","tf","pdf",sep="."),6,6)
    ...
    write.csv(out,paste(genotype2,"vs",genotype1,method,"miRNA","annotated","tf","csv",sep="."),quote=F)
    write.csv(resup,paste(genotype2,"vs",genotype1,method,"miRNA","annotated","tf","hyper","csv",sep="."),quote=F)
    write.csv(resdown,paste(genotype2,"vs",genotype1,method,"miRNA","annotated","tf","hypo","csv",sep="."),quote=F)
}, silent = T)
...
for (n in 1:length(genotype1_tags)){
  for (k in 1:genotype1_reps[n]){
    q = q + 1
    assign(rownames(b)[q], read.table(paste(genotype1,"_",genotype1_tags[n],"_",k,".miRNA.count",sep = ""), header = T, as.is = T, row.names = 1))
  }
}
for (n in 1:length(genotype2_tags)){
  for (k in 1:genotype2_reps[n]){
    q = q + 1
    assign(rownames(b)[q], read.table(paste(genotype2,"_",genotype2_tags[n],"_",k,".miRNA.count",sep = ""), header = T, as.is = T, row.names = 1))
  }
}
for(i in 2:8){
  ...
  try({
    ...
    rownames(sampleDistMatrix) <- rownames(b)
    colors <- colorRampPalette(rev(brewer.pal(9,"Blues")))(255)
    pdf(file=paste(method,"miRNA",i+17,"tf","pdf",sep="."),6,6)
    aheatmap(log2.norm.counts, Rowv=NA, Colv=NA, annCol=df)
    message("Top 1000 completed!")
    plotPCA(rld,ntop=1000,intgroup=c("genotype","design"))
    message("PCA completed!")
    ...
    subset(out, pvalue < pvalueoo & log2FoldChange >= log2(foldchangeoo)) -> resup
    subset(out, pvalue < pvalueoo & log2FoldChange <= -log2(foldchangeoo)) -> resdown
    write.csv(out,paste(genotype2,"vs",genotype1,method,"miRNA",i+17,"tf","csv",sep="."),quote=F)
    write.csv(resup,paste(genotype2,"vs",genotype1,method,"miRNA",i+17,"tf","hyper","csv",sep="."),quote=F)
    write.csv(resdown,paste(genotype2,"vs",genotype1,method,"miRNA",i+17,"tf","hypo","csv",sep="."),quote=F)
  }, silent = T)
}
