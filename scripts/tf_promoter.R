...
for (n in 1:length(genotype1_tags)){
  for (k in 1:genotype1_reps[n]){
    ...
    assign(paste(genotype1,genotype1_tags[n],k,sep="_"), read.table(paste(genotype1,"_",genotype1_tags[n],"_",k,".promoter.count",sep = ""), header = T, as.is = T, row.names = 1))
  }
}
for (n in 1:length(genotype2_tags)){
  for (k in 1:genotype2_reps[n]){
    ...
    assign(paste(genotype2,genotype2_tags[n],k,sep="_"), read.table(paste(genotype2,"_",genotype2_tags[n],"_",k,".promoter.count",sep = ""), header = T, as.is = T, row.names = 1))
  }
}
for(n in 1:p){
...
for(i in 2:8){
  ...
  try({
    ...
    pdf(file=paste(method,"promoter",i+17,"tf","pdf",sep="."),6,6)
    ...
    write.csv(out,paste(genotype2,"vs",genotype1,method,"promoter",i+17,"tf","csv",sep="."),quote=F)
    write.csv(resup,paste(genotype2,"vs",genotype1,method,"promoter",i+17,"tf","hyper","csv",sep="."),quote=F)
    write.csv(resdown,paste(genotype2,"vs",genotype1,method,"promoter",i+17,"tf","hypo","csv",sep="."),quote=F)
  }, silent = T)
}
