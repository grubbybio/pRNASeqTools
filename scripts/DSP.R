...
for (n in 1:length(genotype)){
  for (k in 1:replicates[n]){
    ...
    genotype[n] -> b[p,]
    assign(rownames(b)[p], read.table(paste(genotype[n],"_",k,".promoter.count",sep = ""), as.is = T, row.names = 1)[,1:8])
  }
}
b -> ss
for(n in 1:p){
...
for(i in 2:8){
  ...
  a[apply(a, 1, mean) >= 1,] -> c
  try({
    DESeqDataSetFromMatrix(countData = c, colData = b,design = ~ condition) -> dds
    sizeFactors(dds) <- ss
    ...
    pdf(file=paste(method,"promoter",i+17,"pdf",sep="."),6,6)
    ...
    for(j in 2:length(genotype)){
      ...
      write.csv(out,paste(genotype[j],"vs",genotype[1],method,"promoter",i+17,"csv",sep="."),quote=F)
      write.csv(resup,paste(genotype[j],"vs",genotype[1],method,"promoter",i+17,"hyper","csv",sep="."),quote=F)
      write.csv(resdown,paste(genotype[j],"vs",genotype[1],method,"promoter",i+17,"hypo","csv",sep="."),quote=F)
    }
  }, silent = T)
}
